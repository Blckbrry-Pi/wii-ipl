...
namespace www {
    namespace wiisetting {
        ...
        void DPDCallback2(s32 chan, s32 result) {
            if (result != 1) {
                return;
            }
            gEnableDpd = TRUE;
        ...
        void DPDCallback(s32 chan, s32 result) {
            if (result == 1) {
                KPADEnableDPD(chan);
                KPADSetControlDpdCallback(chan, (KPADCallback*)&DPDCallback2);
            }
        }

        int getKeyboardLanguage(u8 usaLangIdx);
        ...
        int Getter_(WWWJSPluginObj* obj, const char* str, WWWJSPluginValue* val) {
            ...
            if (strPropIdx >= 0) {
                ...
                switch (strPropIdx) {
                    ...
                    case FORM_ID_IP_ADDR: {
                        val->data.jsStr = pString->ip.addr;
                        break;
                    }
                    case FORM_ID_IP_NETMASK: {
                        val->data.jsStr = pString->ip.netmask;
                        break;
                    }
                    case FORM_ID_IP_GATEWAY: {
                    ...
                    case FORM_ID_DIRECT_URL: {
                        val->data.jsStr = pString->netSettingArg;
                        break;
                    }
                    case FORM_ID_SECURITY_KEY: {
                ...
                int staIdx = getBytePropIndex(str);
                if (staIdx >= 0) {
                    val->ty = 2;
                    switch (staIdx) {
                        case WB_ID_SOUND_ID: {
                            sWiiData.data[staIdx] = SCGetSoundMode();
                            saveData = sWiiData.data[staIdx];
                            break;
                        }
                        case WB_ID_NWC24: {
                        ...
                        case WB_ID_DISC_LED: {
                            ...
                            sWiiData.data[WB_ID_DISC_LED] = idleModeInfo.led;
                            break;
                        }
                        case WB_ID_DIS_WIDE: {
                        ...
                        case WB_ID_SEC_Q_SAVE: {
                            sWiiData.data[staIdx] = saveData;
                            ipl::parental::Parental::setSecQ(saveData);
                            break;
                        }
                        case WB_ID_RATE_SAVE: {
                            ipl::parental::Parental::setRating(saveData);
                            sWiiData.data[WB_ID_RATE] = saveData;
                            break;
                        }
                        case WB_ID_OGN: {
                        ...
                        if ((u32)ipl::System::getRegion() == SC_PRODUCT_AREA_USA) {
                            switch (ipl::System::getLanguage()) {
                                case SC_LANG_ENGLISH: {
                                    kbLang = getKeyboardLanguage(0);
                                    break;
                                }
                                case SC_LANG_FRENCH: {
                                    kbLang = getKeyboardLanguage(1);
                                    break;
                                }
                                case SC_LANG_SPANISH: {
                                    kbLang = getKeyboardLanguage(2);
                                    break;
                                }
                            }
        ...
        int Setter_(WWWJSPluginObj* obj, const char* str, WWWJSPluginValue* val) {
            int bytePropIdx;
            bytePropIdx = getBytePropIndex(str);
            if (bytePropIdx == WB_ID_PAGE_ID) {
                sWiiData.data[bytePropIdx] = (u8)val->data.jsDouble;
                switch (sWiiData.data[bytePropIdx]) {
                    case 1: {
                        saveData = 0x20 - (SCGetDisplayOffsetH() + 0x10);
                        break;
                    }
                    case 50: {
                        memset(pString->netSettingArg, 0, sizeof(pString->netSettingArg));
                        break;
                    }
                }
                ...
                switch (bytePropIdx) {
                    ...
                    case WB_ID_SOUND_ID: {
                        sWiiData.data[WB_ID_SE] = sWiiData.data[bytePropIdx] + 10;
                        break;
                    }
                    case WB_ID_LIGHT: {
                        ...
                        bytePropIdx = 0;
                        do {
                            if (WPADIsDpdEnabled(bytePropIdx)) {
                                gEnableDpd = FALSE;
                                gDpdWaitFrm = 0;
                                KPADDisableDPD(bytePropIdx);
                            }
                            KPADSetControlDpdCallback(bytePropIdx, (KPADCallback*)&DPDCallback);
                    ...
                    case WB_ID_SELECT_SEC_KEY: {
                        saveData = ipl::ncd::NCDSetting::getPrivacyMode();
                        ipl::ncd::NCDSetting::setPrivacyMode(sWiiData.data[bytePropIdx]);
                        break;
                    }
                    case WB_ID_BACK_SEC_KEY: {
                        ipl::ncd::NCDSetting::setPrivacyMode(saveData);
                        break;
                    }
                    case WB_ID_AUTO_IP: {
                    ...
                    case WB_ID_RATE_SAVE: {
                        saveData = ipl::parental::Parental::checkRating();
                        break;
                    }
                    case WB_ID_SEC_Q: {
                    ...
                    case WB_ID_SEC_Q_SAVE: {
                        saveData = ipl::parental::Parental::checkSecQ();
                        break;
                    }
                    case WB_ID_USE_ID: {
        ...
        int wiiOSReport(WWWJSPluginObj* objA, WWWJSPluginObj* objB, int valInt, WWWJSPluginValue* valA, WWWJSPluginValue* valB) {
            if (valA->ty == 1) {
                ext_ead::www::print::IPLWWWReport(ext_ead::www::print::WWW_DEBUG, "wiiOSReport Success!!\n");
        ...
        int wiiWriteBack(WWWJSPluginObj* objA, WWWJSPluginObj* objB, int valInt, WWWJSPluginValue* valA, WWWJSPluginValue* valB) {
            ...
            OSReport("WriteBack id:%d data:%d savedata:%d\n", writeBackID, sWiiData.data[writeBackID], saveData);
            switch (writeBackID) {
                case WB_ID_SOUND_ID: {
                ...
                case WB_ID_NWC24: {
                    ...
                    if (!sWiiData.data[WB_ID_NWC24]) {
                        sWiiData.data[WB_ID_STANDBY] = FALSE;
                        sWiiData.data[WB_ID_DISC_LED] = FALSE;
                    } else {
                        sWiiData.data[WB_ID_STANDBY] = TRUE;
                        sWiiData.data[WB_ID_DISC_LED] = TRUE;
                    }

                    wcFlagsIdleInfo.standby = sWiiData.data[WB_ID_STANDBY];
                    wcFlagsIdleInfo.led = sWiiData.data[WB_ID_DISC_LED];
                ...
                case WB_ID_DISC_LED: {
                    ...
                    idleInfo.led = sWiiData.data[WB_ID_DISC_LED];
                ...
                case WB_ID_DIS_POS: {
                    if (valA->data.jsDouble == 0.0) {
                        SCSetDisplayOffsetH((int)(char)(0x10 - saveData));
                        sWiiData.data[WB_ID_DIS_POS] = saveData;
                        sWiiData.data[WB_ID_SET_STRING] = 1;
                    } else {
